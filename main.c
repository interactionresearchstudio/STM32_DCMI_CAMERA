...
static const ShellCommand commands[] = {
  ...
  {"cam_on", cmd_cam_on},
  {"cam_off", cmd_cam_off},
  {"cam_reset", cmd_cam_reset},
  {"cam_capture", cmd_cam_capture},
  {"cam_send", cmd_cam_send},
  {"cam_status", cmd_cam_status},
  {"cam_reg_write", cmd_cam_reg_write},
  {"cam_reg_read", cmd_cam_reg_read},
  {NULL, NULL}
};
...
int main(void) {
  ...
  sdStart(&SD2, NULL);
	   palSetPadMode(GPIOA, GPIOA_PIN2, PAL_MODE_ALTERNATE(7));
	   palSetPadMode(GPIOA, GPIOA_PIN3, PAL_MODE_ALTERNATE(7));
...
static void cmd_cam_off(BaseSequentialStream *chp, int argc, char *argv[]) {
  ...
  /* Stop the clock */
  palSetPadMode(GPIOA, 8, PAL_MODE_OUTPUT_OPENDRAIN);
  palSetPad(GPIOA, 8);
...
static void cmd_cam_capture(BaseSequentialStream *chp, int argc, char *argv[]) {
  ...
  chThdSleepMilliseconds(250);
  dcmiStartReceiveOneShot(&DCMID1, BUFFER_SIZE / 2, ImageBuffer0, ImageBuffer1);
  chThdSleepMilliseconds(250);
...
static void cmd_cam_reg_write(BaseSequentialStream *chp, int argc, char *argv[]) {
  ...
  if (argc != 2) {
    ...
    chprintf(chp, "Enter: cam_reg_write AA BB, where AA is register and BB is the value.\r\n");
    return;
  }
...
static void cmd_cam_reg_read(BaseSequentialStream *chp, int argc, char *argv[]) {
  ...
  uint8_t reg = 0x00;
   ...
   if (argc != 1) {
     chprintf(chp, "Wrong number of arguments!\r\n");
     chprintf(chp, "Enter: cam_reg_read AA, where AA is register location\r\n");
     return;
   }
